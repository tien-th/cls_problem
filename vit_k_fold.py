...
# Training function
def train_one_fold(train_loader, model, criterion, optimizer, device):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
# Validation function
def validate_one_fold(val_loader, model, criterion, device):
    model.eval()
    running_loss = 0.0
    correct = 0
    total = 0
...
def main():
    ...
    data, labels = load_dataset(INDICATOR_CSV)
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(data)):
        ...
        for epoch in range(epochs):
            ...
            train_loss, train_acc, train_f1 = train_one_fold(train_loader, model, criterion, optimizer, device)
            val_loss, val_acc, val_f1 = validate_one_fold(val_loader, model, criterion, device)
            ...
            writer.add_scalar(f'Fold_{fold+1}/Train_Loss', train_loss, global_step)
            writer.add_scalar(f'Fold_{fold+1}/Train_Accuracy', train_acc, global_step)
            writer.add_scalar(f'Fold_{fold+1}/Train_F1', train_f1, global_step)
            writer.add_scalar(f'Fold_{fold+1}/Val_Loss', val_loss, global_step)
            writer.add_scalar(f'Fold_{fold+1}/Val_Accuracy', val_acc, global_step)
            writer.add_scalar(f'Fold_{fold+1}/Val_F1', val_f1, global_step)
